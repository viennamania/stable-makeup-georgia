...
export const PUBLIC_ORDER_API_CALL_LOG_COLLECTION = "publicOrderApiCallLogs";

type InsertPublicOrderApiCallLogInput = {
  route: string;
  method?: string | null;
...
export async function insertPublicOrderApiCallLog(input: InsertPublicOrderApiCallLogInput) {
  const client = await clientPromise;
  const collection = client.db(dbName).collection(PUBLIC_ORDER_API_CALL_LOG_COLLECTION);
...
export async function getPublicOrderApiCallLogs({
  fromDate,
  toDate,
  route = "",
  status = "",
  ...
  if (search) {
    ...
    filters.push({
      $or: [
        ...
        { "requestBody.nickname": regex },
        { "requestBody.walletAddress": regex },
        { "requestBody.mobile": regex },
        { "requestBody.userid": regex },
        { "requestBody.userId": regex },
        { "requestBody.clientid": regex },
        { "requestBody.clientId": regex },
        { "resultMeta.tradeId": regex },
        { "resultMeta.orderId": regex },
