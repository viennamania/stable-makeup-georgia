...
import { updateClientExchangeRateBuy } from "@lib/api/client";
...
export async function POST(request: NextRequest) {
  ...
  const exchangeRateUSDT = isValidExchangeRateMap(body.exchangeRateUSDT);

  if (!exchangeRateUSDT) {
    return NextResponse.json(
      {
        result: null,
        error: "Invalid buy exchange rates",
      },
      { status: 400 },
    );
  }
