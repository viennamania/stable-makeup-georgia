...
import { createSuperadminStoreSettlementWallet } from "@/lib/server/superadmin-store-payment-wallets";
...
const ROUTE_PATH = "/api/superadmin/store-payment-wallets/create";
const SIGNING_PREFIX = "stable-georgia:superadmin:store-payment-wallets:create:v1";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await createSuperadminStoreSettlementWallet({
      storecode,
      audit: {
        route: ROUTE_PATH,
    ...
    const message = error instanceof Error ? error.message : "Failed to create store payment wallet";
