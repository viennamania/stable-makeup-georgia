...
import { assignSuperadminStoreSellerWallet } from "@/lib/server/superadmin-store-payment-wallets";
...
const ROUTE_PATH = "/api/superadmin/store-payment-wallets/update-seller";
const SIGNING_PREFIX = "stable-georgia:superadmin:store-payment-wallets:update-seller:v1";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await assignSuperadminStoreSellerWallet({
      storecode,
      sellerWalletAddress,
      audit: {
        route: ROUTE_PATH,
    ...
    const message = error instanceof Error ? error.message : "Failed to update seller wallet";
    const status =
