...
const normalizeTransactionType = (value: unknown): "" | "deposited" | "withdrawn" => {
...
const buildStats = (logs: any[], key: string) => {
  const map = new Map<string, number>();
  logs.forEach((log) => {
    const value = String(log?.body?.[key] ?? "null");
...
export async function POST(request: NextRequest) {
  ...
  const { limit, range, transactionType } = body || {};
  ...
  return NextResponse.json({
    result,
    transactionStats: buildStats(logs, "transaction_type"),
    matchStats: buildStats(logs, "match"),
  });
}
