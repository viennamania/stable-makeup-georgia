...
const fetchBuyOrders = async () => {
  ...
  if (!address) {
    return (
      <AdminAccessState
        variant="login"
        title={`${store?.storeName || params.center} 센터 운영 로그인 필요`}
        description="센터 거래 이력은 연결된 지갑과 센터 회원 상태를 확인한 뒤에만 열립니다. 먼저 로그인하고 센터 계정을 확인해주세요."
        ...
        secureDescription="센터 운영 화면은 연결된 지갑과 센터 회원 상태가 확인된 계정에서만 열립니다."
        surfaceDescription={`${store?.storeName || params.center} 센터 거래 이력 화면입니다.`}
  ...
  if (
    ...
    return (
      <AdminAccessState
        ...
        description="이 센터 거래 이력은 가맹점 관리자 지갑 또는 승인된 운영 관리자만 조회할 수 있습니다."
        address={address}
        note="센터 회원가입 후 관리자 승인 또는 role=admin 권한이 필요합니다."
        policyTitle="store admin gate"
        policyDescription="store admin wallet / role=admin"
        secureTitle="센터 운영 보호 구역"
        secureDescription="센터 민감 데이터는 가맹점 관리자 지갑 또는 승인된 운영 권한에서만 열립니다."
        surfaceDescription={`${store?.storeName || params.center} 센터 거래 이력 화면입니다.`}
        ...
        <div className="py-0 w-full">
          ...
          <div className="flex flex-col items-start justify-center gap-2">
            ...
            */}
            ...
            <CenterTopMenu lang={params.lang} center={params.center} activeKey="trade-history" />
              ...
              {tableView ? (
                ...
                <div className="w-full overflow-x-auto lg:overflow-x-hidden">
                  ...
                  <table className="w-full table-fixed border-collapse border border-neutral-200 rounded-xl shadow-sm bg-white">

                    <thead
                      className="bg-neutral-900 text-white text-sm font-semibold"
                    >
                      <tr>
                        <th className="p-2 w-[12%] align-top text-left">
                        ...
                        <th className="p-2 w-[14%] align-top text-left">P2P 구매자</th>
                        <th className="p-2 w-[10%] align-top text-left">입금자</th>

                        <th className="p-2 w-[12%] align-top text-right">
                        ...
                        <th className="p-2 w-[11%] align-top text-left">P2P 판매자</th>
                        <th className="p-2 w-[13%] align-top text-left">판매자 입금통장</th>

                        <th className="p-2 w-[10%] align-top text-right">자동입금처리(원)</th>
                        <th className="p-2 w-[18%] align-top text-left">거래상태</th>
